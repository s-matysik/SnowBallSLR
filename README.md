...
| `estimated_recall` | Capture-recapture recall (lower CI bound by default) reaches τ **and** the estimated population has stopped moving. **Advisory by default: reported, but does not terminate a run** |
...
**Why `estimated_recall` is advisory.** The population is closed - the estimand is the set of eligible records existing at the search cutoff - but a run violates a different assumption: at iteration *i* only records within *i* citation hops of the start set are reachable, so the estimate sizes the *reachable frame*, not the population. Early in a run the arms agree almost perfectly on a small frame, N̂ collapses onto the observed count, and estimated recall approaches 1.0 while true recall is still low. Two findings follow from validation. First, requiring N̂ to be stable within 5% for two consecutive iterations raises mean true recall at stop from **0.45** to **0.91** against an achievable ceiling of 0.91, over 270 simulated reviews with known ground truth. Second, a coverage study over 1,620 evaluations found all three estimators systematically low, and their nominal 95% intervals contained the truth in 0% (Chapman) and 14% (Chao1) of the 1,350 evaluations at depths 2-6, where all three are computable on identical rows; over all 1,620 evaluations Chao1 reaches 16.7% and Chapman stays at 0%. A reading at or above τ is therefore not sufficient evidence to end a review, so **this rule reports rather than stops**: its decision and rationale are recorded every iteration, but termination comes from the other rules. Set `authoritative: true` to opt into recall-based termination, and `min_stable_iterations: 0` to disable the stability guard.
